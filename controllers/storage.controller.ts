...
export class StorageController {
	...
	async postUpload(req: Request): Promise<APIRes<unknown>> {
		...
		if (file.size > parseInt(process.env.MAX_FILE_SIZE as string))
			throw new PayloadTooLargeException("file too large");

		const folder = decideFolder(file.type);
		...
		await fetch(process.env.DISCORD_WEBHOOK_URL as string, {
			body: JSON.stringify({
				content: `${process.env.BASE_URL}/storage/uploads/${path} uploaded`,
			}),
			headers: {
		...
		return {
			data: `${process.env.BASE_URL}/storage/uploads/${path}`,
